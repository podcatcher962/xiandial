...
想在电脑上双击就用（不用开浏览器、独立窗口、录音更完整）的话：

**https://github.com/podcatcher962/xiandial/releases/latest** → 下载 `XianDial.exe`（约 20 MB，免安装）

> 发布资产用的是英文名 `XianDial.exe`，跟中文名「拾声.exe」是同一个文件。
